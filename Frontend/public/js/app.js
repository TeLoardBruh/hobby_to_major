...
! function () {
    ...
    function goToResults() {
        ...
        var top3 = sortedMajors.slice(0).sort(function (a, b) {
        ...
        for (var i = 0; i < top3.length; i++) {
            // html += ['<p1><a href="', majors[top3[i]].url, '" target="_blank">', top3[i], '</a></p1>'].join('');

            html += [`<p${i} id="subject${i}">`, top3[i], `</p${i}>`, '<br>'].join('');
            console.log(typeof(top3[i]));
